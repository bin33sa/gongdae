                                ...
                                <tr>
                                    ...
                                    <td class="admin-td txt-center">
                                        <c:choose>
                                            ...
                                            <c:when test="${dto.grade == 'BRONZE'}"><span class="grade-badge grade-bronze">BRONZE</span></c:when>
                                            <c:otherwise>
                                    ...
                                    <td class="admin-td txt-center">
                                        <c:choose>
                                            <c:when test="${dto.enabled == 1}"><span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1">정상</span></c:when>
                                            <c:otherwise><span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-2 py-1">정지</span></c:otherwise>
                                        </c:choose>
                                    </td>
...
document.getElementById('btn-excel-guest').addEventListener('click', function() {
    ...
    const wb = XLSX.utils.table_to_book(table, {sheet: "게스트목록"});
    XLSX.writeFile(wb, "공대생_게스트_목록.xlsx");
});
</script>
</body>
</html>