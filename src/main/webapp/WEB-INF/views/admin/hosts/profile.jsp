...
<head>
    ...
    <title>공대생 Admin - 호스트 상세 정보</title>
</head>
    ...
    <div class="admin-body d-flex flex-grow-1">
        ...
        <main class="admin-content flex-grow-1 p-4" style="width: 0;">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h3 class="fw-bold mb-2">호스트 상세 정보</h3>
                    <p class="mb-0 text-muted">호스트(사업자)의 상세 정보를 확인하고 계정 상태를 관리합니다.</p>
                </div>
                <div>
                    <button type="button" class="btn btn-outline-secondary" onclick="location.href='${pageContext.request.contextPath}/admin/hosts/list?page=${page}';">
            ...
            <div class="dashboard-box mb-4">
                ...
                <div class="table-responsive">
                    <table class="table text-main mb-0 align-middle">
                        <tr>
                            <th class="admin-th" style="width: 15%;">파트너번호</th>
                            <td class="admin-td text-muted" style="width: 35%;">#H-${dto.member_id}</td>
                            <th class="admin-th" style="width: 15%;">아이디</th>
                            <td class="admin-td" style="width: 35%;">${dto.login_id}</td>
                        </tr>
                        <tr>
                            <th class="admin-th">대표자 이름</th>
                            <td class="admin-td fw-bold">${dto.name}</td>
                            <th class="admin-th">상호명 (법인명)</th>
                            <td class="admin-td text-warning fw-bold">${empty dto.nickname ? '<span class="text-muted" style="opacity:0.5; margin-left:40px;">-</span>' : dto.nickname}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="admin-td border-0">
                                <c:choose>
                                    <c:when test="${dto.enabled == 1}"><span class="status-normal">정상 (활성)</span></c:when>
                                    <c:otherwise><span class="status-banned">잠금 (정지)</span></c:otherwise>
                                </c:choose>
                            </td>
                            <th class="admin-th border-0">최근 정보 수정일</th>
                            <td class="admin-td text-muted border-0">${dto.update_at}</td>
                        </tr>
                    </table>
                </div>

                <div class="mt-4 pt-4 d-flex justify-content-end border-top border-secondary border-opacity-25">
        ...
        <div class="modal-content bg-dark text-white border-secondary shadow-lg">
            ...
            <div class="modal-body p-4">
                <form name="memberStatusForm" id="memberStatusForm">
                    ...
                    <div class="mb-3">
                        <label class="form-label small-txt text-muted">대상 호스트</label>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_code" id="statusCode" class="form-select" style="background-color:rgba(0,0,0,0.2); border-color:#374151;" onchange="selectStatusChange(this)">
                            ...
                            <option value="3">불법적인 매장 운영</option>
                            <option value="4">고객 클레임 누적</option>
                            <option value="5">수수료/정산 미납</option>
